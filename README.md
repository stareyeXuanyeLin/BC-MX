...
**已知原版限制**：位置广播由地图视图运行循环（`ChatRoomMapViewUpdatePlayerSync`）消费同步标志后发出。目标玩家停留在聊天视图时，本地位置已更新但不会广播，传送对其他玩家不可见。

**接收端增强**：当目标玩家同样安装本插件时，插件会在目标端 hook 原版消息处理流程，收到传送指令后强制广播一次位置，因此目标玩家停留在聊天视图也能完成传送。未安装插件的目标玩家需要处于地图视图，否则发送端会在约 2.5 秒后提示传送未生效。
