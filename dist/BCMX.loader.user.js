// ==UserScript==
// @name         Bondage Club - BCMX（正式版加载器）
// @name:zh-CN   Bondage Club - 地图功能强化（正式版加载器）
// @namespace    https://github.com/stareyeXuanyeLin/BC-MX
// @version      0.1.2
// @description  Fetches and executes the latest BCMX from the official branch with a privileged request and network fallback.
// @description:zh-CN 每次进入页面时通过特权请求获取并执行 main 分支的最新 BCMX，并在网络故障时自动切换备用源。
// @author       林宣夜＆佩菈
...
// @noframes
// @run-at       document-end
// @downloadURL  https://raw.githubusercontent.com/stareyeXuanyeLin/BC-MX/main/dist/BCMX.loader.user.js
// @updateURL    https://raw.githubusercontent.com/stareyeXuanyeLin/BC-MX/main/dist/BCMX.loader.user.js
// ==/UserScript==
...
(function () {
    ...
    const LOADER_GUARD = "__BCMX_LOADER__";
    const EXECUTION_MARKER = "__BCMX_CORE_EVALUATED__";
    if (pageWindow[LOADER_GUARD]) return;
    pageWindow[LOADER_GUARD] = true;

    const cacheKey = Date.now();
    const corePath = "stareyeXuanyeLin/BC-MX/main/dist/BCMX.user.js";
    const sources = Object.freeze([
        `https://raw.githubusercontent.com/${corePath}?timestamp=${cacheKey}`,
        `https://cdn.jsdelivr.net/gh/stareyeXuanyeLin/BC-MX@main/dist/BCMX.user.js?timestamp=${cacheKey}`,
        `https://fastly.jsdelivr.net/gh/stareyeXuanyeLin/BC-MX@main/dist/BCMX.user.js?timestamp=${cacheKey}`,
        `https://gcore.jsdelivr.net/gh/stareyeXuanyeLin/BC-MX@main/dist/BCMX.user.js?timestamp=${cacheKey}`,
    ]);

    function fail(message, detail) {
        delete pageWindow[LOADER_GUARD];
        console.error(`[BCMX Loader] ${message}`, detail || "");
    }

    function validCore(source) {
        return typeof source === "string"
            && source.includes('const MOD_NAME = "BCMX"')
    ...
    function executeCore(source, sourceURL) {
        ...
        delete pageWindow[EXECUTION_MARKER];
        console.info(`[BCMX Loader] latest core executed from ${sourceURL}.`);
    }
        ...
        GM_xmlhttpRequest({
            ...
            onload(response) {
                if (response.status < 200 || response.status >= 300 || !validCore(response.responseText)) {
                    console.warn(`[BCMX Loader] invalid response from ${sourceURL} (HTTP ${response.status}); trying the next source.`);
                    loadFrom(attempt + 1);
                    return;
                }
                try {
            ...
            onerror(error) {
                console.warn(`[BCMX Loader] request failed for ${sourceURL}; trying the next source.`, error);
                loadFrom(attempt + 1);
            },
            ontimeout() {
                console.warn(`[BCMX Loader] request timed out for ${sourceURL}; trying the next source.`);
                loadFrom(attempt + 1);
            },
        });
    }
